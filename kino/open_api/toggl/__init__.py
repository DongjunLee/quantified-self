...
class Toggl:
    ...
    def requestRaw(self, endpoint, parameters=None):
        ...
        if parameters is None:
            return urllib.request.urlopen(
                urllib.request.Request(endpoint, headers=self.headers)
            ).read()
        else:
            ...
            return urllib.request.urlopen(
                urllib.request.Request(endpoint, headers=self.headers)
            ).read()  # make request and read the response
    ...
    def postRequest(self, endpoint, parameters=None):
        ...
        if parameters is None:
            ...
            return urllib.request.urlopen(
                urllib.request.Request(endpoint, data=data, headers=self.headers)
            ).read()  # make request and read the response
    ...
    def getClientProjects(self, client_project_id):
        ...
        """
        return self.request(
            Endpoints.CLIENTS + "/{0}/projects".format(client_project_id)
        )
