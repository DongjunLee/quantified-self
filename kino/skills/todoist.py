...
class TodoistManager(object):
    ...
    def schedule(self, channel=None):
        ...
        task_text = (
            MsgResource.TODOIST_OVERDUE(task_count=overdue_task_count)
            + "\n"
        ...
        specific_task_list = list(
            filter(lambda x: x[2] != "anytime", self.__get_task(today_task))
        )

        attachments = MsgTemplate.make_todoist_task_template(specific_task_list)
    ...
    def remain_task(self):
        ...
        attachments = MsgTemplate.make_todoist_task_template(remain_task_list)
    ...
    def __get_karma_trend(self):
        user = self.todoist_api.user.login(
            Config.open_api.todoist.ID, Config.open_api.todoist.PASSWORD
        )
    ...
    def __get_event_counts(self):
        ...
        for log in activity_log_list:
            event_date = arrow.get(log["event_date"], "DD MMM YYYY HH:mm:ss Z").to(
                "Asia/Seoul"
            )
    ...
    def __get_label_names(self, task):
        labels = task["labels"]
        return [
            self.todoist_api.labels.get_by_id(label).data["name"] for label in labels
        ]
    ...
    def get_today_tasks_with_label(self):
        ...
        return list(
            map(
                lambda x: {
                    "content": x["content"],
                    "label": self.__get_label_name_by_id(x["labels"][0]),
                },
                label_tasks,
            )
        )
